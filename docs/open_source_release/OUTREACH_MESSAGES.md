...
这组消息用于邀请真实开发者和企业评测负责人，不用于群发广告。每次只发送给一个明确匹配的对象，并记录是否完成首次运行。
...
不需要提供任何私有数据，也不涉及投资建议。你的反馈会帮助我确定第一批生态场景包和运行时改进方向。
...
如果团队正在比较多个模型、Agent 编排或工具调用策略，我们可以把一个真实工作流建模成受约束世界，运行至少 10 次可重放 episode，输出 evidence → action → event → settlement → outcome 链路和对比报告。

公开运行时可先用无密钥回放验证；私有试点再按数据边界、部署方式和验收标准报价。标准试点人民币 29,800 元起，不承诺模型性能、投资收益或业务结果。

试点说明：<https://github.com/tonyhyworld/TraceArena/blob/main/docs/open_source_release/PILOT_ONE_PAGER.md>
申请入口：<https://github.com/tonyhyworld/TraceArena/issues/new?template=agent-evaluation-pilot.yml>

如果方向匹配，请回复一个工作流、权威结果和目标日期；我们先给范围与验收草案，再决定是否进入正式报价。

## 每次发送后记录

- 对象与渠道；
- 是否打开 Demo/Quickstart；
- 首次结果耗时；
- 一个具体卡点或价值反馈；
- 是否愿意贡献场景包；
- 是否形成评测试点机会。
