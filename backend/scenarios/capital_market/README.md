# 资本市场专业评测 Public Edition v0.3.0

两位投资 Agent 在相同本金、数据边界与工具权限下接受统一评测：**可选市场研究 + 模拟成交 + 可审计结算**。系统计入佣金与滑点，并按风险调整后超额收益形成结果。

这是一个可公开运行的投资策略专业评测场景包，不是自动交易系统。默认配置使用 Mock Agent、无 Key 回放和模拟账本；用户可以自行配置模型 Provider，并在合法授权下启用只读市场研究工具。场景不会连接券商，也不会提交真实订单。
...
- 默认观察窗口：墙钟 3600 秒；安全帽 `tick_limit: 120`
- 报告指标：组合收益、基准超额收益、最大回撤、换手率、交易成本
- 主评测指标：`超额收益率 - 0.5 × 最大回撤`
